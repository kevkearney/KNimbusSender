...
bool KnimbusControlValueHelper::SetNewControlValues(WeatherControlMsg &weatherControl, WeatherControlMsg &newWeatherControlValues) {
  ...
  if (weatherControl.EnableDisturbers != newWeatherControlValues.EnableDisturbers) {
    weatherControl.EnableDisturbers = newWeatherControlValues.EnableDisturbers;
    Serial.print(F("Enable Disturbers changed to: "));
